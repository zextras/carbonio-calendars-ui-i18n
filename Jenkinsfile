...
pipeline {
	...
	environment {
		...
		TRANSLATIONS_REPOSITORY_NAME = "zextras/com_zextras_zapp_calendar"
	}
	stages {
			...
			parallel {
				stage('Build package') {
					...
					steps {
						...
						stash includes: 'pkg/com_zextras_zapp_calendar.zip', name: 'zimlet_package_unsigned'
					}
				}
				stage('Build documentation') {
					...
					steps {
						script {
							...
							stash includes: 'docs/website/build/com_zextras_zapp_calendar/', name: 'doc'
						}
					}
				}
			}
			...
			parallel {
				stage('Deploy documentation') {
					...
					steps {
						script {
							unstash 'doc'
							doc.rm file: "iris/zapp-calendar/${BRANCH_NAME}"
							doc.mkdir folder: "iris/zapp-calendar/${BRANCH_NAME}"
							doc.upload file: "docs/website/build/com_zextras_zapp_calendar/**", destination: "iris/zapp-calendar/${BRANCH_NAME}"
						}
					}
				}
				stage('Deploy Beta on demo server') {
					...
					steps {
						script {
							unstash 'zimlet_package'
							sh 'unzip pkg/com_zextras_zapp_calendar.zip -d deploy'
							iris.rm file: 'com_zextras_zapp_calendar/*'
							// iris.mkdir folder: 'com_zextras_zapp_calendar'
							iris.upload file: 'deploy/*', destination: 'com_zextras_zapp_calendar/'
						}
					}
				}
			}
