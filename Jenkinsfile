...
def createBuild(sign) {
    ...
    if (sign) {
        dir("artifact-deployer") {
            ...
            sh(script: """#!/bin/bash
                ./sign-zextras-zip ../pkg/com_zextras_zapp_calendar.zip
            """)
        }
    }
}
...
//                 }
		...
		stage("Release automation") {
            ...
            steps {
                ...
                createBuild(true)
                archiveArtifacts(
                    artifacts: "pkg/com_zextras_zapp_calendar.zip",
                    fingerprint: true
                )
                stash(
                    includes: "pkg/com_zextras_zapp_calendar.zip",
                    name: 'zimlet_package'
                )
                ...
                script {
                    ...
                    doc.upload(
                        file: "docs/website/build/com_zextras_zapp_calendar/**",
                        destination: "iris/zapp-calendar/$BRANCH_NAME"
                    )
                }
